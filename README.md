# AI Breast Cancer subtype Transcriptomics

An undergraduate bioinformatics project that explores breast cancer transcriptomic data using Python, public genomic datasets, data visualization, and introductory machine learning techniques.

## Project Objectives
- Analyze publicly available breast cancer gene expression datasets
- Perform data cleaning and preprocessing
- Visualize biological patterns
- Apply introductory machine learning methods
- Learn computational approaches used in modern biomedical research

## Technologies Used
- Python
- Pandas
- NumPy
- Matplotlib
- Scikit-learn
- Google Colab

## Dataset
Publicly available breast cancer transcriptomic dataset (NCBI GEO).

## Skills Demonstrated
- Bioinformatics
- Data Analysis
- Machine Learning
- Scientific Visualization
- Computational Biology

## Future Improvements
- Differential gene expression analysis
- Pathway enrichment analysis
- Deep learning models
- Interactive visualizations

---

*This project was developed as part of my self-learning journey into bioinformatics and computational biology.*
